...
// Array of slides with class "holidayPix"
var slideshowElems = document.getElementsByClassName("holidayPix");
...
function getDescription(path) {
  ...
  // file is of format "yymmdd_picture-description" where yy is always two-digit year,
  //   mm is always two-digit month, and dd is always two-digit day
  ...
  if (index >= 0 && path.lastIndexOf('.jpg') > index) {
    ...
    if (/^[0-9]{6}[_T]{1}/.test(file)) {
      var date = file.substring(0, 6);

      if (/^[0-9]{6}_[C-Z]{1}[0-9]{7}/.test(file)) {
        // Found digital camera picture name ("yymmdd_Annnnnnn")
        result = file.substring(7, 15);
        index = 15;
      } else if (/^[0-9]{6}_[A-B]{1}[0-9]{3}-[0-9]{2}/.test(file)) {
        // Found film camera picture name ("yymmdd_Annn-nn") NOT YET USED!
        result = file.substring(7, 14);
        index = 14;
      } else if (/^[0-9]{6}T[0-9]{6}/.test(file)) {
        // Found cell phone camera picture name ("yymmddTnnnnnn") NOT YET USED!
        result = file.substring(0, 6) + file.substring(7, 13);
        index = 13;
      }
      ...
      if (result.length > 0) {
        ...
        // Get English date from yymmdd
        var dateStr = "";
        var month = [ "January", "February", "March", "April",
                      "May", "June", "July", "August", "September",
                      "October", "November", "December" ][+date.substring(2, 4) - 1];
        dateStr = month + " " + (date.charAt(4) == '0' ? date.charAt(5) : date.substring(4, 6)) + ", 20" + date.substring(0, 2);
        if (dateStr.length > 0) {
